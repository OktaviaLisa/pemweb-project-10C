...
@php
    // Pastikan variabel jenis dikirim dari controller (Bootcamp atau Private Mentoring)
    $jenis = $jenis ?? 'Bootcamp'; // default Bootcamp kalau kosong

    // Tentukan route store dan update sesuai jenis
...
@section('content')
<div class="pt-36 max-w-3xl mx-auto bg-white p-8 rounded shadow">
    ...
    <form action="{{ isset($kelas) ? route($routeUpdate, $kelas->idKelas) : route($routeStore) }}" method="POST" enctype="multipart/form-data">
        ...
        <!-- Gambar -->
        <label for="gambar" class="block font-semibold mb-1">Gambar (kosongkan jika tidak ingin diubah)</label>
