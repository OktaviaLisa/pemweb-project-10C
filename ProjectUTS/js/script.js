...
hamburger.addEventListener('click', function () {
    this.classList.toggle('hamburger-active'); // Jika punya animasi X
    navMenu.classList.toggle('hidden'); // Menyembunyikan atau menampilkan menu
});
...
function openConfirmModal(id) {
    console.log('openConfirmModal called with id:', id); // Debugging log
    // Menampilkan modal
    const modal = document.getElementById('confirmModal');
    modal.classList.remove('hidden'); // Menampilkan modal

    // Set action pada tombol hapus
    const deleteButton = document.getElementById('deleteButton');
    deleteButton.onclick = function() {
        console.log('Hapus data dengan id:', id); // Debugging log
        // Redirect ke halaman hapus.php dengan idKeranjang yang dipilih
        window.location.href = "hapus.php?id=" + id;
    };

    // Menambahkan event listener untuk menutup modal saat mengklik area luar modal
    modal.onclick = function(event) {
        if (event.target === modal) {
            closeModal(); // Tutup modal jika area luar diklik
        }
    };
}
